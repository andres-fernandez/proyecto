...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // d3.json('data/departamentos_sanitized.json').then(function (mapa) {
  //   departamentos.attr('fill', 'none')
  //     .selectAll('path')
  //     .data(mapa.features)
  //     .join('path')
  //     .attr('d', geoPathGenerator)
  ...
  d3.csv('./data/bibliotecas.csv').then(data => {
    ...
    heat.data(data.map(d => {                                        // Set data of [[x, y, value], ...] format
  ...
  console.log('Bibliotecas populares en Argentina render in: ' + (end - start) + 'ms.');
